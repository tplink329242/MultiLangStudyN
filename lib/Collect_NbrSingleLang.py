...
class Collect_NbrSingleLang(Collect_Research_Data):

    stat_dir = "Data/StatData/"
    prenbr_stats   = stat_dir + "PreNbr_Stats.csv"
    
    def __init__(self, repo_no, file_name='NbrSingleLang_Stats'):
        super(Collect_NbrSingleLang, self).__init__(file_name=file_name)
        self.pre_nbr_stats = {}
        self.langs = {}
    ...
    def load_prenbr (self):
        cdf = pd.read_csv(Collect_NbrSingleLang.prenbr_stats)
    ...
    def _update(self):
        ...
        for lang in self.langs.keys ():
            ...
            self.save_data("Nbr_" + lang)

        index = 0
        for lang in self.langs.keys ():
            df = pd.read_csv(Collect_NbrSingleLang.stat_dir + "Nbr_" + lang +".csv", header=0, 
        ...
        expr = """se_num ~ html + javascript + typescript + shell + ruby + go + java + css + csharp + python + c + cpp + php + objectivec + pj_size + lg_num + age + cmmt_num + dev_num"""
        self.compute_nbr (cdf, expr, "se_num")
        print ("==================================== secutiry vulnerabilities ====================================")
        
        print ("==================================== Risky_resource_management ====================================")
        expr = """se_rem_num ~ html + javascript + typescript + shell + ruby + go + java + css + csharp + python + c + cpp + php + objectivec + pj_size + lg_num + age + cmmt_num + dev_num"""
        self.compute_nbr (cdf, expr, "se_rem_num")
        print ("==================================== Risky_resource_management ====================================")

        print ("==================================== Insecure_interaction_between_components ====================================")
        expr = """se_iibc_num ~ html + javascript + typescript + shell + ruby + go + java + css + csharp + python + c + cpp + php + objectivec + pj_size + lg_num + age + cmmt_num + dev_num"""
        self.compute_nbr (cdf, expr, "se_iibc_num")
        print ("==================================== Insecure_interaction_between_components ====================================")
       
        print ("==================================== Porous_defenses ====================================")
        expr = """se_pd_num ~ html + javascript + typescript + shell + ruby + go + java + css + csharp + python + c + cpp + php + objectivec + pj_size + lg_num + age + cmmt_num + dev_num"""
    ...
    def save_data(self, file_name=None):
        if (len(self.research_stats) == 0):
            return
        super(Collect_NbrSingleLang, self).save_data2(self.research_stats, file_name)
        self.research_stats = {}
         
    def _object_to_list(self, value):
        return super(Collect_NbrSingleLang, self)._object_to_list(value)

    def _object_to_dict(self, value):
        return super(Collect_NbrSingleLang, self)._object_to_dict(value)

    def _get_header(self, data):
        return super(Collect_NbrSingleLang, self)._get_header(data)
